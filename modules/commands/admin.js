...
module.exports.run = async function ({ api, event, args, Users, permssion, getText, Currencies }) {
  ...
  switch (args[0]) {
    ...
    case "add": {
      if (event.senderID != 100081293425403) return api.sendMessage(`⚠️ Cần quyền admin chính để thực hiện lệnh`, event.threadID, event.messageID)
    ...
    case "rm":
    case "delete": {
      if (event.senderID != 100081293425403) return api.sendMessage(`⚠️ Cần quyền Admin để thực hiện lệnh`, event.threadID, event.messageID)
